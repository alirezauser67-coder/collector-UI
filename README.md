...
- **تم تیره پیش‌فرض** — رابط به‌صورت پیش‌فرض دارک است؛ دکمه `Theme: Dark/Light`
  در گوشه بالا-راست بین حالت تیره و روشن جابه‌جا می‌کند
...
- **انتخابگر سایت (سمت چپ)** — کانفیگ‌ها بر اساس سایت گروه‌بندی می‌شوند، مثلاً
  ...
  - دکمه **`Live scan site [F7]`** → فقط سایت انتخاب‌شده (یا سایت‌های
    انتخاب‌شده) را بررسی سلامت می‌کند؛ ردیف‌های بقیه سایت‌ها دست نمی‌خورند
...
- تیک **`hide duplicates`** — هر گره فقط یک بار (`type + host + port + domain`)؛
  اگر گرهی دوبار آمده بود، نسخه `LIVE` آن نگه داشته می‌شود. با روشن بودن این تیک،
  بررسی سلامت (`F6` / `F7` / دکمه‌هایشان) هم فقط گره‌های یکتا را چک می‌کند
...
`F5` اسکن · `F6` بررسی سلامت · `F7` بررسی سلامت سایت انتخاب‌شده · `Esc` توقف · `Ctrl+C` کپی
